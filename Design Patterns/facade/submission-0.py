...
class DriveThruFacade:
    ...
    def takeOrder(self, orderContents: str, takeOut: bool) -> Food:
        ...
        order_placed= self.cashier.takeOrder(orderContents, takeOut)
        self.prepared_food = self.chef.prepareFood(order_placed)
        if takeOut== True:
            return self.kitchenStaff.packageOrder(self.prepared_food)
        else:
            return self.prepared_food
